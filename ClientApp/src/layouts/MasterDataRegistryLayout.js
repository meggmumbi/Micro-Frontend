...
import masterDataRegistryItems from "../components/sidebar/masterDataRegistryItems";
import Sidebar from "../components/sidebar/Sidebar";
import Footer from "../components/Footer";
// import Settings from "../components/Settings";
import { UserInformation, SiteContext } from "../index.js";
import _ from 'lodash';
import NavbarGlobalFund from "../components/navbar/NavbarGlobalFund.js";
...
const MasterDataRegistryLayout = ({ children }) => {
  const userInfo = useContext(UserInformation).userInformation;
  ...
  const mdrFilteredRoutes = _.find(filteredRoutes, { path: "master-data-registry" })

  // Filter the dashboardItems based on the filteredRoutes
  const filteredDashboardItems = masterDataRegistryItems.map((dashboardItem) => {

    const filteredPages = dashboardItem.pages.filter((page) => {
      const itemPath = page.href.replace(/\//g, "").replace('master-data-registry', '')
      const itemInRoutes = mdrFilteredRoutes.children.some(
        (route) => route.path.replace(/\//g, "") === itemPath
      );

      // if (itemInRoutes) {
      //   console.log('itemPathitemPathitemPathchild', itemPath)
      //   if (page.children) {
      ...
      //   return true;
      // }
      return itemPath === '' ? true : itemInRoutes;
    });
...
export default MasterDataRegistryLayout;
